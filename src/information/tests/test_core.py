...
from src.baml_client.types import Information as BamlInformation, InfromationItem, PostType
...
async def test_get_information_from_text():
    ...
    with patch('src.information.core.call_llm', new=AsyncMock(return_value=mock_baml_information)):
        # 调用 get_information_from_text 方法
        tag: List[str] = []
        result = await information_core.get_information_from_text("请告诉我关于Python的信息", tag)
...
async def test_get_information_from_content_text():
    ...
    with patch('src.information.core.call_llm', new=AsyncMock(return_value=mock_baml_information)):
        ...
        tag: List[str] = []
        result = await information_core.get_information_from_content("请告诉我关于文本处理的信息", tag)
...
async def test_empty_information():
    ...
    with patch('src.information.core.call_llm', new=AsyncMock(return_value=mock_baml_information)):
        # 调用 get_information_from_text 方法
        tag: List[str] = []
        result = await information_core.get_information_from_text("这是一段没有信息内容的文本", tag)
