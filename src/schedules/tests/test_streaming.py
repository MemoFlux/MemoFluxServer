...
async def test_gen_schedule_from_text_stream():
    ...
    with patch('src.schedules.core.call_llm_stream', return_value=mock_stream_data()):
        # 收集所有流式数据
        stream_results = []
        async for partial_data in schedule_core.gen_schedule_from_text_stream("今天下午两点开会"):
...
async def test_gen_schedule_from_image_stream():
    ...
    with patch('src.schedules.core.call_llm_stream', return_value=mock_empty_stream_data()):
        stream_results = []
        async for partial_data in schedule_core.gen_schedule_from_image_stream(mock_image):
            stream_results.append(partial_data)
...
async def test_gen_schedule_from_content_stream_with_text():
    ...
    with patch('src.schedules.core.call_llm_stream', return_value=mock_stream_data()):
        stream_results = []
        async for partial_data in schedule_core.gen_schedule_from_content_stream("测试文本"):
            stream_results.append(partial_data)
...
async def test_gen_schedule_from_content_stream_with_image():
    ...
    with patch('src.schedules.core.call_llm_stream', return_value=mock_stream_data()):
        stream_results = []
        async for partial_data in schedule_core.gen_schedule_from_content_stream(mock_image):
            stream_results.append(partial_data)
...
async def test_stream_error_handling():
    ...
    with patch('src.schedules.core.call_llm_stream', side_effect=mock_error_stream):
        with pytest.raises(Exception, match="模拟错误"):
            async for partial_data in schedule_core.gen_schedule_from_text_stream("错误测试"):
                pass 