...
from src.common.streaming_output.document_example import DocumentProcessor, MockBamlDocument, MockBamlDocumentSection
from src.common.streaming_output.schemas import Document, DocumentSection, PartialStreamingDocument
...
class TestDocumentProcessor:
    ...
    async def test_call_llm_with_generic_content(self, processor):
        """测试普通内容的 LLM 调用"""
        generic_text = "这是一份普通的文档内容，没有特定的类型标识。包含了一些基本信息和描述。"
    ...
    def test_convert_to_schema_success(self, processor):
        ...
        mock_sections = [
            ...
            MockBamlDocumentSection("第二部分", "第二部分的内容", 2, ["标签2", "标签3"])
        ]
    ...
    def test_postprocess_result_adds_default_tags(self, processor):
        ...
        sections_without_tags = [
            ...
            DocumentSection(title="有标签章节", content="内容", level=1, tags=["已有标签"])
        ]
    ...
    async def test_error_handling_in_llm_call(self, processor):
        """测试 LLM 调用中的错误处理"""
        # 模拟 _call_llm 中发生异常
        with patch.object(processor, '_call_llm', side_effect=RuntimeError("模拟LLM错误")):
            with pytest.raises(RuntimeError, match="模拟LLM错误"):
                await processor.process_from_text("这是一个足够长的测试内容，用于模拟错误处理")
    ...
    async def test_error_handling_in_streaming(self, processor):
        ...
        with patch.object(processor, '_call_llm_stream', side_effect=mock_error_stream):
            with pytest.raises(Exception, match="流式处理错误"):
                chunks = []
                async for chunk in processor.process_from_text_stream("这是一个足够长的测试内容，用于模拟流式错误处理"):
                    chunks.append(chunk)
