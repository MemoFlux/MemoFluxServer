...
from src.common.streaming_output.document_example import document_processor, DocumentProcessor
...
class TestStreamingOutputIntegration:
    ...
    async def test_unified_interface_consistency(self):
        """测试统一接口的一致性"""
        test_content = "这是一个用于测试统一接口一致性的文档内容，包含足够的信息来进行完整的处理。"
    ...
    async def test_concurrent_processing_performance(self):
        ...
        # 并发处理所有内容
        tasks = [
            document_processor.process_from_text(content)
    ...
    async def test_error_propagation_and_recovery(self):
        """测试错误传播和恢复"""
        # 测试处理错误的传播
        with patch.object(document_processor, '_call_llm', 
                         side_effect=RuntimeError("模拟处理错误")):
        ...
        # 验证错误后处理器仍然能正常工作
        normal_content = "错误恢复测试：这是一个正常的文档内容，用于验证处理器在错误后仍能正常工作。"
    ...
    async def test_input_validation_across_interfaces(self):
        ...
        for invalid_input in invalid_inputs:
            ...
            # 测试流式接口
            with pytest.raises(ValueError, match="输入验证失败"):
                async for chunk in document_processor.process_from_text_stream(invalid_input):
                    pass
            
            with pytest.raises(ValueError, match="输入验证失败"):
                async for chunk in document_processor.process_from_content_stream(invalid_input):
                    pass
    ...
    async def test_comprehensive_workflow(self):
        ...
        for scenario in test_scenarios:
            ...
            # 流式处理
            chunks = []
            async for chunk in document_processor.process_from_text_stream(scenario["content"]):
                chunks.append(chunk)
