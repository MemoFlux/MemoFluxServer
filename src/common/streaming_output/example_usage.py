...
async def demo_non_streaming_processing():
    ...
    test_cases = [
        ...
        {"name": "短文本处理", "content": "这是一个简短的测试文本。", "params": {}},
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            result = await document_processor.process_from_text(
                test_case["content"], **test_case["params"]
            )
    ...
    try:
        ...
        async for chunk in document_processor.process_from_text_stream(test_content):
            ...
            if chunk.sections and chunk.sections.value:
                print(
                    f"  章节数: {len(chunk.sections.value)} (状态: {chunk.sections.state})"
                )
...
async def demo_error_handling():
    ...
    error_cases = [
        {"name": "空文本输入", "content": "", "expect_error": True},
        {"name": "过短文本输入", "content": "短", "expect_error": True},
        {
            "name": "正常文本输入",
