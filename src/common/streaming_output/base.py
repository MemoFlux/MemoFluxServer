...
class LLMContentProcessor(Generic[T, T_Stream], ABC):
    ...
    # 公开接口 - 流式处理
    async def process_from_text_stream(
        self, text: str, **kwargs
    ) -> AsyncGenerator[T_Stream, None]:
        """
        流式从文本生成结果
        ...
        try:
            ...
            async for chunk in self._call_llm_stream(processed_text, **kwargs):
                ...
                try:
                    # 尝试验证数据块
                    if hasattr(chunk, "model_validate") and hasattr(
                        chunk, "model_dump"
                    ):
                    ...
                    self.logger.error(
                        f"数据块字段: {list(chunk.model_fields.keys()) if hasattr(chunk, 'model_fields') else 'No model_fields'}"
                    )  # type: ignore
                    ...
                    if hasattr(chunk, "model_fields"):
                        for field_name in getattr(chunk, "model_fields", {}):
                            if hasattr(chunk, field_name):
                                ...
                                self.logger.error(
                                    f"字段 {field_name}: {field_value} (类型: {type(field_value)})"
                                )
    ...
    async def process_from_image_stream(
    ...
    async def process_from_content_stream(
    ...
    def _convert_to_schema(
        self, baml_result: Any, original_content: str, **kwargs
    ) -> T:
        """
