...
import uuid
from typing import Union, AsyncGenerator, Any, Optional, List
...
class MockBamlDocument:
    """模拟 BAML 返回的文档对象"""
    def __init__(self, title: str, summary: str, category: str, sections: list, language: str = "zh"):
        self.title = title
...
class MockBamlDocumentSection:
    """模拟 BAML 返回的章节对象"""
    def __init__(self, title: str, content: str, level: int = 1, tags: Optional[List[str]] = None):
        self.title = title
...
class DocumentProcessor(LLMContentProcessor[Document, PartialStreamingDocument]):
    ...
    async def _call_llm_stream(self, content: Union[str, Image], **kwargs) -> AsyncGenerator[PartialStreamingDocument, None]:
        """
    ...
    def _convert_to_schema(self, baml_result: MockBamlDocument, original_content: str, **kwargs) -> Document:
        """
    ...
    def _postprocess_result(self, result: Document, **kwargs) -> Document:
        ...
        self.logger.info(f"文档后处理完成，标题：{result.title}，章节数：{len(result.sections)}")
        return result
