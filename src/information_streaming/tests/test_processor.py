...
from src.baml_client.types import (
    ...
    InfromationItem as BamlInformationItem,
)
...
def sample_baml_result():
    ...
    return BamlInformation(
        title="测试信息标题",
        information_items=[
            BamlInformationItem(header="测试项1", content="这是测试项1的内容")
        ],
...
@pytest.mark.asyncio
async def test_process_from_text(
    information_processor, sample_text, sample_tags, sample_baml_result
):
    """测试从文本处理信息"""
    # 模拟 BAML 调用
    with patch.object(
        ...
        "_call_llm",
        new=AsyncMock(return_value=sample_baml_result),
    ):
        result = await information_processor.process_from_text(
            sample_text, tags=sample_tags
        )
...
@pytest.mark.asyncio
async def test_process_from_text_stream(
    information_processor, sample_text, sample_tags
):
    """测试从文本流式处理信息"""
    # 创建示例流式结果
    ...
    # 模拟 BAML 流式调用
    with patch.object(
        information_processor, "_call_llm_stream"
    ) as mock_call_llm_stream:
        # 创建异步生成器模拟
        ...
        # 测试流式处理
        count = 0
        async for partial_result in information_processor.process_from_text_stream(
            sample_text, tags=sample_tags
        ):
            count += 1
...
async def test_validate_input(information_processor, sample_text, sample_tags):
    """测试输入验证"""
    # 有效输入
    assert information_processor._validate_input(sample_text, tags=sample_tags)

    # 无效输入（太短）
    assert not information_processor._validate_input("短", tags=sample_tags)

    # 有效图像输入
    mock_image = AsyncMock(spec=Image)
    assert information_processor._validate_input(mock_image, tags=sample_tags)
...
async def test_preprocess_content(information_processor, sample_text, sample_tags):
    """测试内容预处理"""
    # 文本预处理
    processed_text = information_processor._preprocess_content(
        sample_text, tags=sample_tags
    )
    ...
    # 图像预处理
    mock_image = AsyncMock(spec=Image)
    processed_image = information_processor._preprocess_content(
        mock_image, tags=sample_tags
    )
