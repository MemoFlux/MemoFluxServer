...
class TestScheduleProcessorIntegration:
    ...
    async def test_process_from_text_integration(self):
        ...
        mock_schedule = BamlSchedule(
            title="测试日程", category="测试", tasks=[mock_task]
        )

        with patch(
            "src.schedules_streaming.processor.b.ScheduleManager",
            new=AsyncMock(return_value=mock_schedule),
        ):
    ...
    async def test_process_from_text_stream_integration(self):
        ...
        from src.baml_client.types import (
            StreamingSchedule as BamlStreamingSchedule,
            Task as BamlTask,
        )
    ...
    async def test_process_from_image_integration(self):
        ...
        mock_schedule = BamlSchedule(
            title="图像日程", category="测试", tasks=[mock_task]
        )
