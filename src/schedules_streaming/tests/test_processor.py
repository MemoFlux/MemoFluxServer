...
def sample_baml_schedule():
    ...
    return BamlSchedule(title="明日计划", category="日常安排", tasks=[task1, task2])
...
class TestScheduleProcessor:
    ...
    def test_convert_to_schema(self, schedule_processor, sample_baml_schedule):
        ...
        result = schedule_processor._convert_to_schema(
            sample_baml_schedule, original_content
        )
    ...
    async def test_call_llm_text(self, schedule_processor, sample_baml_schedule):
        """测试文本 LLM 调用"""
        with patch(
            "src.schedules_streaming.processor.b.ScheduleManager",
            new=AsyncMock(return_value=sample_baml_schedule),
        ):
    ...
    async def test_call_llm_stream_text(self, schedule_processor, sample_baml_schedule):
        ...
        with patch(
            "src.schedules_streaming.processor.b.stream.ScheduleManagerStream",
    ...
    async def test_process_from_text_stream(
        self, schedule_processor, sample_baml_schedule
    ):
        """测试从文本流式处理"""
        ...
        with patch(
            ...
            chunks = []
            async for chunk in schedule_processor.process_from_text_stream(
                "测试文本内容"
            ):
                chunks.append(chunk)
