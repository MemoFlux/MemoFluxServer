...
from src.baml_client.types import Knowledge as BamlKnowledge, KnowledgeItem, Node, RelationShip
...
async def test_get_knowledge_from_text():
    ...
    with patch('src.knowledge.core.call_llm', new=AsyncMock(return_value=mock_baml_knowledge)):
        # 调用 get_knowledge_from_text 方法
        tag: List[str] = []
        result = await knowledge_core.get_knowledge_from_text("请告诉我关于Python的知识", tag)
...
async def test_get_knowledge_from_content_text():
    ...
    with patch('src.knowledge.core.call_llm', new=AsyncMock(return_value=mock_baml_knowledge)):
        ...
        tag: List[str] = []
        result = await knowledge_core.get_knowledge_from_content("请告诉我关于文本处理的知识", tag)
...
async def test_empty_knowledge():
    ...
    with patch('src.knowledge.core.call_llm', new=AsyncMock(return_value=mock_baml_knowledge)):
        # 调用 get_knowledge_from_text 方法
        tag: List[str] = []
        result = await knowledge_core.get_knowledge_from_text("这是一段没有知识内容的文本", tag)
