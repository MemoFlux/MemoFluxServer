...
from src.baml_client.types import (
    ...
    KnowledgeItem as BamlKnowledgeItem,
)
...
def sample_baml_result():
    ...
    return BamlKnowledge(
        ...
        knowledge_items=[
            BamlKnowledgeItem(
                id=1, header="测试项1", content="这是测试项1的内容", node=None
            )
        ],
...
async def test_process_from_text(
    knowledge_processor, sample_text, sample_tags, sample_baml_result
):
    """测试从文本处理知识"""
    # 模拟 BAML 调用
    with patch.object(
        knowledge_processor, "_call_llm", new=AsyncMock(return_value=sample_baml_result)
    ):
        result = await knowledge_processor.process_from_text(
...
async def test_process_from_text_stream(knowledge_processor, sample_text, sample_tags):
    ...
    with patch.object(knowledge_processor, "_call_llm_stream") as mock_call_llm_stream:
        ...
        # 测试流式处理
        count = 0
        async for partial_result in knowledge_processor.process_from_text_stream(
            sample_text, tags=sample_tags
        ):
            count += 1
...
async def test_validate_input(knowledge_processor, sample_text, sample_tags):
    """测试输入验证"""
    # 有效输入
    assert knowledge_processor._validate_input(sample_text, tags=sample_tags)

    # 无效输入（太短）
    assert not knowledge_processor._validate_input("短", tags=sample_tags)

    # 有效图像输入
    mock_image = AsyncMock(spec=Image)
    assert knowledge_processor._validate_input(mock_image, tags=sample_tags)
...
async def test_preprocess_content(knowledge_processor, sample_text, sample_tags):
    """测试内容预处理"""
    # 文本预处理
    processed_text = knowledge_processor._preprocess_content(
        sample_text, tags=sample_tags
    )
    ...
    # 图像预处理
    mock_image = AsyncMock(spec=Image)
    processed_image = knowledge_processor._preprocess_content(
