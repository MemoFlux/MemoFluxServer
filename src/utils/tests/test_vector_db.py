...
def mock_qdrant_client():
    ...
    mock_search_hits = [
        rest.ScoredPoint(
            id=str(i), version=1, score=0.9 - i * 0.1, payload={"text": f"hit_{i}"}
        )
        for i in range(3)
    ]
...
async def test_create_and_initialize_new_collection(
    mock_qdrant_constructor,
    mock_utils_config,
    mock_get_embeddings,
    mock_qdrant_client,
    mock_keys_embedding_response,
):
...
async def test_initialize_with_existing_populated_collection(
    ...
    mock_qdrant_client.get_collection.assert_awaited_once_with(
        collection_name=MOCK_COLLECTION_NAME
    )
...
async def test_search_successfully(
    ...
    search_response_data = JinaEmbeddingData(
        object="embedding", embedding=query_embedding, index=0
    )
    search_response = JinaEmbeddingResponse(
        object="list",
        data=[search_response_data],
        model="jina-embeddings-v4",
        usage=JinaEmbeddingUsage(total_tokens=5),
    )
    ...
    mock_qdrant_client.search.assert_awaited_once_with(
        collection_name=MOCK_COLLECTION_NAME, query_vector=query_embedding, limit=3
    )
